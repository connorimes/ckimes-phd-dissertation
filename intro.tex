...
However, voltage and frequency are correlated \cite{LeSueur2010}, meaning their values in \eqnref{bg-power} cannot be adjusted independently.
...
\appref{tools} details some of these portable tools.
...
\item \textbf{Energy Savings}: Using an offline oracle, we verify that POET achieves near-optimal energy consumption (\eg 1.3\% on an Intel-powered tablet, 2.9\% on an ARM big.LITTLE system), which includes POET's runtime overhead.
