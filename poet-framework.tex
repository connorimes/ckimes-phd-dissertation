...
%    \Require $b(t)$ \Comment{given by \eqnref{kalman-filter} (\appref{KalmanFilter})}
    \Require $\tau$ \Comment{timing constraint, given by application}
  ...
  \Comment{compute time to spend in each config in pair}
    \State $\tau_o = \tau - \tau_u$
  ...
  \Comment{compare energy to best found so far}
    ...
    \Return $schedule$ \Comment{time to spend in each configuration}
  \end{algorithmic}
