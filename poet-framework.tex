...
The application informs the runtime of its job \emph{performance goal}.
...
The application provides a \emph{performance goal}, or work rate, $r_d$ which is easily computed from a known workload size $\omega$ and desired latency goal (deadline) $\tau$:
\begin{equation}
  r_d = \frac{\omega}{\tau}
...
% POET users do not need to understand control theory or Kalman filtering.
...
% To meet the performance requirement, POET ensures that the application completes $\omega$ jobs in the next time interval $\tau$, with $\omega = \tau \cdot s(t) \cdot b(t)$.
...
% Given this information, POET schedules for each configuration $c$ an execution time $\tau_c$, ensuring that the $\omega$ iterations complete and the total energy consumption is minimized.
...
\begin{algorithm}[t]
  ...
  \State $\omega_u = \left\lfloor \omega \cdot \frac{s_o \cdot (s_u - s(t))}{s(t) \cdot (s_u - s_o)} \right\rfloor$
...
Note that the algorithm takes the floor of the value computed for the first configuration set---since the value must be a whole number, this ensures that the actual speedup is not less than the target.
Subsequently, \algoref{poet-optimal} loops over all possible pairs of configurations, with one from each set, to determine how much time should be spent in each configuration given speedup constraint.
