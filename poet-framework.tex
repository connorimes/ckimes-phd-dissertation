...
Therefore, {\em \algoref{poet-optimal} will find a minimal-energy schedule}.
% In practice this process can be sped up by considering only
% Pareto-optimal (in performance/power) configurations.  Sorting into
% Pareto-optimal takes $O(C \text{lg} C)$ time and need only be done
% once.  After sorting, configurations can be indexed by a bucketed hash
% table with speedup as the key.  Given a sufficiently large table this
% reduces the per control invocation complexity to constant time.
