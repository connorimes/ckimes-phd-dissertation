...
\figref{poet-mape-server} presents the MAPE values for each application for the four latency targets.
...
\figref{poet-server-ee} presents the ratio of energy consumption to optimal for each latency target, where unity is optimal and values greater than 1 show energy consumed over optimal.
...
Later fluctuations are a result of variability in the input video (\app{x264} inputs exhibit high variance -- see \figref{poet-server-variation}).
...
For example, the \app{bodytrack} executions used a window size of $20$ on the embedded systems, but we used a size of $50$, and later $40$, for evaluations on the manycore server-class system for the same application.
...
\figref{poet-bodytrack-w20} demonstrates the results of using a window size of $20$, which is too small, for a latency target of about 50\% capacity (the same one used the analysis of multiple applications).
